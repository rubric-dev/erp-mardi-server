...
@SqlResultSetMapping(
        ...
        classes = @ConstructorResult(
                ...
                columns = {
                        @ColumnResult(name = "fullProductCode", type = String.class),
                        ...
                        @ColumnResult(name = "depletionRatePlan", type = Integer.class),
...
@NamedNativeQuery(
        ...
        query = """
select
    d.full_prod_cd as fullProductCode
    ...
    ,coalesce((select sum(sales_qty) from daily_sales dsa where dsa.full_prod_cd = d.full_prod_cd),0) as salesQty -- 판매량
...
from daily_stock d
    left join(
        select full_prod_cd, sum(sales_qty) as sales , avg(sales_qty) as salesAvg
        from daily_sales
        where "date" between :from and :to
            and (:distributionChannel is null or dist_channel = :distributionChannel)
        group by full_prod_cd
    ) sa on sa.full_prod_cd = d.full_prod_cd
    LEFT JOIN (
        SELECT full_prod_cd,sum(expected_inbound_qty) as expectedInboundQty, sum(stock_qty) as todayQty, sum(expected_outbound_qty) as accExpectedOutboundQty
        FROM daily_stock
        WHERE "date" = (select max("date") from daily_stock d )
        group by full_prod_cd
    ) e ON d.full_prod_cd = e.full_prod_cd
    LEFT JOIN (
        SELECT full_prod_cd, sum(stock_qty) as firstQty
        FROM daily_stock
        WHERE "date" = :from
        group by full_prod_cd
    ) f ON d.full_prod_cd = f.full_prod_cd
where (:products is null or d.full_prod_cd  in (:products))
    and d.date between :from and :to
    and (:warehouseId is null or d.warehouse_id = :warehouseId)
group by d.prod_cd, d.color_cd, d.full_prod_cd, f.firstQty, e.expectedInboundQty, e.accExpectedOutboundQty, e.todayQty, sa.sales, sa.salesAvg
;
"""
)
